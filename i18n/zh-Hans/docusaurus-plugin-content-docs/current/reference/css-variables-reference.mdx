...
:::tip
如果应用是通过 `starter.createApp().render()` 启动的，这些变量会自动注入到 `:root`。如果你是手动挂载组件层，请保留 `@vef-framework/components` 的 `ConfigProvider`，这样全局变量才会正常输出。
:::
...
`globalCssVars` 是 `@vef-framework/components` 导出的 JS/TS 别名层。它把 CSS 变量映射成 camelCase 字段，例如：
