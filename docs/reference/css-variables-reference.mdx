...
:::tip
If your app is bootstrapped through `starter.createApp().render()`, these variables are already injected onto `:root`. If you mount the component layer manually, keep `@vef-framework/components` `ConfigProvider` in place so the global variables are emitted.
:::
...
`globalCssVars` is the JS/TS alias layer exported by `@vef-framework/components`. It maps CSS variable names into camelCase fields, for example:
